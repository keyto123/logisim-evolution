...
public class RamIO extends Mem {
	...
	void configurePorts(Instance instance) {
		...
		ps[CS].setToolTip(
				new SimpleStringGetter("Byte selects: each 0 disables access to one byte of the addressed word."));
	...
	public void propagate(InstanceState state) {
		...
		if (triggered) {
			...
			if ((ka_value.toIntValue() == 1) && (irq_pending.toIntValue() != 1)) { // hardware read one byte into
																					// memory.
			...
			if ((state.getPortValue(OE).toIntValue() == 1) && (state.getPortValue(ADDR).toIntValue() == 0xFF0004)) {
	...
	public void paintInstance(InstancePainter painter) {
		...
		GraphicsUtil.drawCenteredText(g, "TTY", bds.getX() + bds.getWidth() / 4 * 3,
				bds.getY() + 5 + bds.getHeight() / 4);
		GraphicsUtil.drawCenteredText(g, "CNTL", bds.getX() + bds.getWidth() / 4 * 3,
				bds.getY() + 25 + bds.getHeight() / 4);

		g.drawRect(bds.getX() + bds.getWidth() / 3 * 2, bds.getY() + bds.getHeight() / 2 + 20, bds.getWidth() / 3,
				bds.getHeight() / 2 - 30);
		GraphicsUtil.drawCenteredText(g, "KEYB", bds.getX() + bds.getWidth() / 4 * 3,
				bds.getY() + bds.getHeight() / 4 * 3 - 5);
		GraphicsUtil.drawCenteredText(g, "CNTL", bds.getX() + bds.getWidth() / 4 * 3,
				bds.getY() + bds.getHeight() / 4 * 3 + 15);

		GraphicsUtil.drawCenteredText(g, "RAM + IO CNTL", bds.getX() + bds.getWidth() / 4 + 25,
