...
class MemState implements InstanceData, Cloneable, HexModelListener {
	...
	private void setBits(int addrBits, int dataBits) {
		...
		// I do subtraction in the next two conditions to account for possibility of
		// overflow
	...
	public void paint(Graphics g, int leftX, int topY) {
		...
		for (int row = 0; row < ROWS; row++) {
			...
			if (isValidAddr(addr)) {
				...
				GraphicsUtil.drawText(g, font, StringUtil.toHexString(getAddrBits(), (int) addr), x - 2, y + yoffs,
				...
				if (addr == curAddr) {
					...
					for (int i = 0; i < 4; i++) {
						...
						GraphicsUtil.drawText(g, font, StringUtil.toHexString(8, (val >> (8 * i)) & 0xff),
								x0 + DATA_WIDTH_PER_BYTE / 2, y + yoffs, GraphicsUtil.H_CENTER,
								GraphicsUtil.V_BASELINE);
						x0 -= DATA_WIDTH_PER_BYTE;
					}
					g.setColor(Color.BLACK);
				} else {
					GraphicsUtil.drawText(g, font, StringUtil.toHexString(dataBits, val), x + entryWidth / 2, y + yoffs,
