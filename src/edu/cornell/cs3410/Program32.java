...
public class Program32 extends InstanceFactory {
	...
	private void drawBox(Graphics g, Bounds bds, Color color) {
		...
		g.fillRect(bds.getX() + ARROW_WIDTH, bds.getY() + 5, ACOL_WIDTH, 20 * NUM_ROWS + 10);
		g.fillRect(bds.getX() + ARROW_WIDTH, bds.getY() + 5, BOX_WIDTH, 20 * NUM_ROWS + 10);
		g.setColor(color);
		g.drawRect(bds.getX() + ARROW_WIDTH, bds.getY() + 5, ACOL_WIDTH, 20 * NUM_ROWS + 10);
		g.drawRect(bds.getX() + ARROW_WIDTH, bds.getY() + 5, BOX_WIDTH, 20 * NUM_ROWS + 10);
	...
	public void paintInstance(InstancePainter painter) {
		...
		GraphicsUtil.drawText(g, "PC", bds.getX() + 2, bds.getY() + CHIP_DEPTH - 12, GraphicsUtil.H_LEFT,
				GraphicsUtil.V_CENTER);
		GraphicsUtil.drawText(g, "Op", bds.getX() + CHIP_WIDTH - 2, bds.getY() + CHIP_DEPTH / 2, GraphicsUtil.H_RIGHT,
				GraphicsUtil.V_CENTER);
	...
	private void drawState(InstancePainter painter) {
		...
		else if (state.isUndefinedPC())
			arrowcolor = Color.GRAY;
		else if (state.haveCodeFor(state.pc))
			arrowcolor = Color.BLUE;
		else
			arrowcolor = Color.BLUE; // Color.YELLOW;
		...
		for (int i = pc - (NUM_ROWS - 1) / 2; i <= pc + (NUM_ROWS - 1) / 2; i++) {
			...
			GraphicsUtil.drawText(g, font, StringUtil.toHexString(32, i * 4), bds.getX() + ARROW_WIDTH + 2,
					bds.getY() + 20 * j + 20 / 2 + 10, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
			GraphicsUtil.drawText(g, font, state.decode(i), bds.getX() + ARROW_WIDTH + ACOL_WIDTH + 1,
					bds.getY() + 20 * j + 20 / 2 + 10, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
			g.setColor(Color.BLACK);
		}
	}
	...
	private static class ContentsAttribute extends Attribute<Listing> {
		...
		public java.awt.Component getCellEditor(Window source, Listing value) {
			if (source instanceof Frame) {
				...
				if (state != null)
					state.setProject(proj); // will set on Program/AutoProgram
			}
		...
		@Override
		public String toDisplayString(Listing value) {
		...
		public Listing parse(String value) {
			try {
				...
				JOptionPane.showMessageDialog(null,
						"The contents of the Program chip could not be read: " + e.getMessage(),
